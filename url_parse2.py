#coding:utf-8
"""
来自Django源码urls包的resolvers.py
这个模块主要是介绍几个Django的正则表达式匹配工具
用于建立url与响应视图函数的关系
"""
...
# 这个类是下面几乎所有regex类的归宿
# 在正则匹配完毕后 将对应的视图函数和url名传进来
# 注意视图函数传进来之后 存储其路径 方便直接调用
class ResolverMatch(object):
    def __init__(self, func, args, kwargs, url_name=None, app_names=None, namespaces=None):
        ...
        # 存储函数的路径
        # 这样就能在返回对象的时候返回的函数就是一个可以直接调用的函数
...
# 这个函数wsgi的父类里面定义 并且在wsgi类中调用的
# urlconf是我们总的那个urls.py
# 也就是创建项目之后自动生成的那个urls.py里面的那个列表
...
# 这个是大头
# 它的resolve函数可以将一个完整的url直接解析出ResolverMatch对象
# 区别于上面那个类 上面那个类只能解析应用urls模块里面定义的正则与视图关系
# 也就是说 上面那个类只能解析子路径
# 这个类的resolve函数是先解析了父路径 然后调用楼上的resolve解析出子路径
# 然后综合它们的信息 返回ResolverMatch对象
class RegexURLResolver(LocaleRegexProvider):
    ...
    # 这个函数的应用见BaseHandler类的get_response函数
    # 先就项目url（区分于应用url，它们包里都有url文件)进行分析和匹配
    # 然后就应用url进行进一步的匹配 最后返回两者信息的综合 
    def resolve(self, path):
        path = force_text(path)  # path may be a reverse_lazy object
        tried = []
        # 就项目url模块里面的正则表达式进行匹配
        match = self.regex.search(path)
        if match:
            # 匹配成功
            # 进一步匹配url里面子路径
            new_path = path[match.end():]
            # 注意pattern本身是RegexURLPattern对象
            for pattern in self.url_patterns:
                try:
                    # 调用RegexURLPattern对象里的resolve方法
                    # 看看子路径是否能与其pattern中的一个正则匹配上
                    # 如果能匹配上 就返回一个ResolverMatch对象
                    ...
                    if sub_match:
                        ...
                        # 重新实例化一个ResolverMatch对象
                        # 这一步的必要性在于 把父路径与子路径携带的信息综合起来
